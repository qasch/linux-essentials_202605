...
Im Verzeichnis `/proc` befinden sich Informationen zu laufenden Prozessen. Lass dir den Inhalt des Verzeichnisses mit `ls` anzeigen. Was bedeuten die darin enthaltenen Verzeichnisse mit den Nummern? Erkennst du eine der Nummern wieder? Schau dort doch einmal hinein und wirf einen Blick in die Dateien `status`, `cmdline` und `environ`.
