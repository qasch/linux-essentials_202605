...
1. Erstellt ein ZIP-Archiv `backup.zip`, das alle Dateien eines Verzeichnisses enthält, also beispielsweise alle `bigfile` Dateien. 

2. Wie unterscheidet sich das Verhalten von `zip` im Vergleich zu `tar` mit `gzip`, `bzip2` und `xz`? Ihr könnt (falls ihr wollt) auch hier einmal die Kompressionsgeschwindigkeit und resultierende Dateigrösse vergleichen.
